...
%Intial Conditions
init.phi1 = 3*pi/180;   %Initial angular offset of the lower segment from vertical, in rad
init.phi1d = 0;         %Intial angular velocity of the lower segment around ankle, in rad/s
init.phi2 = 3*pi/180;   %Initial angular offset of the upper segment from lower segment, in rad
...
i2 = 1;                         %initialize counter for success loop (second method)
loops = 25;                    %Number of iterations of each torque to try
stableflag = zeros(loops);      %Initialize array to indicate where in looping success occured
stableflag2 = zeros(loops);     %Initialize array to indicate where in looping success occured (second method)
...
for k1 = 1:loops                    %Loop to iterate across ankle torques
    ...
    for k2 = 1:loops                %Loop to iterate across hip torqes for one ankle torque
        ...
        %% Approach 2 - Run ODE45 for single time steps and check each result - Up to 60x slower
        in = q0;
        t2 = zeros(100,1);
        step2 = 0.005;
        tic
        for i = 1:100
            out = solve_step(segment, in, t2(i), step2);
            t2(i+1) = out.t;
            in = [out.phi1/180*pi; out.phi1d/180*pi; out.phi2/180*pi; out.phi2d/180*pi];
            if (precision >= out.phi1) && (out.phi1 >= -precision) && (out.phi2 >= -precision) && (out.phi2 <= precision)
                stableflag2(k1,k2) = 1;
                success2(i2,1) = tankle;
                success2(i2,2) = thip;
                success2(i2,3) = thip/tankle;
                success2(i2,4) = out.t;
                i2 = i2+1;
                break
            end
        end
        time((k1-1)*loops+k2,2) = toc;
        time((k1-1)*loops+k2,4) = i;
    end
end
...
fprintf("Average time for Method 2 loop execution: %1.4f sec \n",mean(time(:,2)));
fprintf("Total time for Method 1 loop execution: %1.4f sec \n",sum(time(:,1)));
fprintf("Total time for Method 2 loop execution: %1.4f  sec \n",sum(time(:,2)));
if (exist('success','var') == 1)
    ...
    fprintf(meantxt,mean(1),mean(2), mean(3), mean(4)*.5/100);
