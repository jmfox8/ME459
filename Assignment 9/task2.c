#include <stdlib.h>
#include <stdio.h>
#include <time.h>
#include "matmul.h"

#define NSEC_PER_MSEC 1000000

int main(int argc, char* argv[]){
        if (argc != 2){             // If to make sure that a value is passed through command line
        printf("provide dimension for square matrix\n");
        return 1;
    }
    struct timespec start1, end1; // Declare the variables used to capture time values
    int n = atoi(argv[1]);                      // Capture Array size from command line
    double *A = (double*)malloc((n * n) * sizeof(double));  // Allocate memory for arrays
    double *B = (double*)malloc((n * n) * sizeof(double));
    double *C = (double*)malloc((n * n) * sizeof(double));

        for(int i = 0; i<n*n; i++){                 // Loop to populate arrays with random values between -1 and 1
        A[i]=rand();
        B[i]=rand();
        A[i] = A[i]/RAND_MAX * 2 - 1;
        B[i] = B[i]/RAND_MAX * 2 - 1;
    }

clock_gettime(CLOCK_MONOTONIC, &start1);        // Capture timing for the function
mmul1(A,B,C,n);
clock_gettime(CLOCK_MONOTONIC, &end1);

double elapsed_sec_1 = end1.tv_sec - start1.tv_sec; // Get time in seconds of function execution
long double elapsed_nsec_1 = end1.tv_nsec - start1.tv_nsec; // Get time in nanoseconds of function execution
double elapsed_msec_1 = elapsed_sec_1 * 1000 + elapsed_nsec_1/(double)NSEC_PER_MSEC; //Convert time of execution to milliseconds

printf("%f \n %f \n",elapsed_msec_1,C[(n*n)-1]);

free(A);
free(B);
free(C);

}